...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> handleNotificationDomainException(
            NotificationDomainException ex
    ) {
        log.warn("Notification 도메인 예외 발생: errorCode={}, message={}",
                ex.getErrorCode(), ex.getMessage());

        return ResponseEntity
                .status(ex.getHttpStatus())  // ✅ Enum에서 HTTP 상태 코드 가져오기
                .body(ex.getMessage());
    }
    ...
    public ResponseEntity<String> methodArgumentTypeMismatchException(MethodArgumentTypeMismatchException ex) {
        log.warn("[Bad Request] 파라미터 타입 오류 - name={}, value={}", ex.getName(), ex.getValue());
        return ResponseEntity
                .status(HttpStatus.BAD_REQUEST)
